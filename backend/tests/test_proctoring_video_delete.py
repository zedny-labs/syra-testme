...
from app.services import supabase_storage, vimeo_media
...
def test_missing_identifier_is_a_noop_success():
    assert asyncio.run(routes_public._delete_video_storage_object({"provider": "vimeo"})) is True
...
def test_provider_failure_propagates_false(monkeypatch):
    ...
    ok = asyncio.run(routes_public._delete_video_storage_object({"provider": "vimeo", "uid": "abc"}))

    assert ok is False


def test_legacy_cloudflare_provider_honestly_reports_failure():
    # Cloudflare video storage support was removed from this deployment; a
    # video whose meta still says provider=cloudflare (saved before the
    # migration to Vimeo) can no longer be reached to delete remotely. The
    # dispatcher must report that honestly (False) rather than claiming
    # success for a delete that didn't happen.
    ok = asyncio.run(routes_public._delete_video_storage_object({"provider": "cloudflare", "uid": "legacy-uid"}))

    assert ok is False


def test_legacy_cloudflare_provider_with_no_uid_is_a_noop_success():
    ok = asyncio.run(routes_public._delete_video_storage_object({"provider": "cloudflare"}))
...
def test_admin_deletes_video_event_and_writes_audit_log(monkeypatch):
    ...
    try:
        ...
        event = _create_video_event(
            db, attempt=attempt, meta={"provider": "vimeo", "uid": "vid-1", "uri": "/videos/vid-1"}
        )
...
def test_admin_delete_reports_storage_failure_but_still_removes_reference(monkeypatch):
    ...
    try:
        ...
        exam = _create_exam(db, owner=admin)
        attempt = _create_attempt(db, exam=exam, learner=learner)
        event = _create_video_event(db, attempt=attempt, meta={"provider": "vimeo", "uid": "vid-2"})
...
def test_admin_delete_404s_for_wrong_attempt(monkeypatch):
    ...
    try:
        ...
        event = _create_video_event(db, attempt=attempt_a, meta={"provider": "vimeo", "uid": "vid-3"})
...
def test_admin_delete_rejects_non_owning_admin(monkeypatch):
    ...
    try:
        ...
        attempt = _create_attempt(db, exam=exam, learner=learner)
        event = _create_video_event(db, attempt=attempt, meta={"provider": "vimeo", "uid": "vid-4"})
